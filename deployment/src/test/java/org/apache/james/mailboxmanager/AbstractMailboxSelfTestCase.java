...
public abstract class AbstractMailboxSelfTestCase extends TestCase {
    ...
    public void testGetFirstUnseen() throws MailboxManagerException,
        ...
        mailbox.setFlags(new Flags(Flags.Flag.DELETED), true, false,
                MessageRangeImpl.uidRange(1, 3), FetchGroupImpl.MINIMAL,
                mailboxSession);
    ...
    protected void assertEventCount(MailboxListenerCollector collector,
            long[] added, long[] flags, long[] expunged) {
        assertEquals("added size", added.length, collector.getAddedList(true)
                .size());
        assertEquals("flags size", added.length, collector.getFlaggedList(true)
                .size());
        assertEquals("deleted size", added.length, collector.getExpungedList(
                true).size());

    }

    protected void checkMessageResults(long[] uids, int[] msns,
            Iterator messages) {
